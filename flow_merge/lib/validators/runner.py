...
class ValidationRunner:
    def __init__(self, raw_data: Dict[str, Any], env, logger):
        ...
        self.directory_settings = self.validate(
            DirectorySettings, ["directory_settings"]
        )
        self.tokenizer_settings = self.validate(TokenizerSettings, ["tokenizer"])
        self.base_model, self.models = self.validate(
            ModelSettings, ["base_model", "models"]
        )._unpack()
    ...
    # {
    #     "sources": [{"model": "model_1", "base_model": True}, {"model": "model_2"}],
    #     "range": [0, 2],
    #     "layers": ["mlp"],
    #     "merge_method": "slerp",
    # },
