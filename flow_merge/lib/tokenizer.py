...
class TokenizerLoader:
    @staticmethod
    def load_all_tokenizers(models: List[Model], base_model: Model) -> Dict[Model, PreTrainedTokenizerBase]:
        all_tokenizers = {}
...
class TokenizerValidator:
    @staticmethod
    def check_tokenizers_for_differences(tokenizers: Dict[Model, PreTrainedTokenizerBase]) -> bool:
        differences_found = False

        for (model_a, tokenizer_a), (model_b, tokenizer_b) in combinations(tokenizers.items(), 2):
            differences_found |= TokenizerValidator._compare_tokenizer_vocabs(model_a, tokenizer_a, model_b, tokenizer_b)
            differences_found |= TokenizerValidator._compare_special_tokens(model_a, tokenizer_a, model_b, tokenizer_b)
            differences_found |= TokenizerValidator._compare_added_tokens_encoders(model_a, tokenizer_a, model_b, tokenizer_b)
    ...
    def _compare_tokenizer_vocabs(
        ...
        if vocab_a != vocab_b:
            print(f"Tokenizer for model {model_a} has different vocab compared to model {model_b}.")
            return True
        return False
    ...
    def _compare_special_tokens(
        ...
        if special_tokens_a != special_tokens_b:
            print(f"Tokenizer for model {model_a} has different special tokens compared to model {model_b}.")
            return True
        return False
    ...
    def _compare_added_tokens_encoders(
        ...
        if added_tokens_encoder_a != added_tokens_encoder_b:
            print(f"Tokenizer for model {model_a} has different added tokens encoder compared to model {model_b}.")
            return True
        return False
...
class TokenizerMerger:
        ...
        for input_id, added_token in added_tokens.items():
            ...
            if token in merged_added_tokens:
                if merged_added_tokens[token] != added_token and token not in duplicate_added_tokens:
                    print(f"Token {token} added with multiple different settings, using the first one by default.")
    ...
    def _create_merged_tokenizer(
        ...
        for special_token_type, special_token in merged_special_tokens.items():
            if special_token_type == ADDITIONAL_SPECIAL_TOKENS_KEY and isinstance(special_token, list):
                print(f"Adding additional special tokens: {special_token}.")
                merged_tokenizer.add_special_tokens({ADDITIONAL_SPECIAL_TOKENS_KEY: special_token})
            else:
                print(f"Overriding {special_token_type} with {special_token}. When a conflict occurs, the last one takes priority.")
...
class InputIDsMapper:
    ...
    def create_input_ids_mappings(
        ...
        print("Creating input ids mappings for interpolation of `embed_tokens` and `lm_head` layers.")
        input_ids_mappings = {}
    ...
    def get_vocab_size(model: Model, trust_remote_code: bool) -> Optional[int]:
        try:
            ...
            print(f"Can't get vocab size for {model}: {e}")
            return None


def construct_appropriate_tokenizer(
        base_model: Model, tokenizer_mode, all_tokenizers: Dict[Model, PreTrainedTokenizerBase]
) -> PreTrainedTokenizerBase:
...
def get_merge_tokenizer(models: List[Model], base_model: Model, tokenizer_mode: str) -> Tokenizer:
    all_tokenizers = TokenizerLoader.load_all_tokenizers(models, base_model)

    if not TokenizerValidator.check_tokenizers_for_differences(all_tokenizers):
        print(f"No differences in tokens or vocab among tokenizers. Using {base_model.path} for the tokenizer.")
        return Tokenizer(tokenizer=all_tokenizers[base_model])

    print("Different tokens or vocab among tokenizers. Building the tokenizer for the merged model.")

    merge_tokenizer = construct_appropriate_tokenizer(base_model, tokenizer_mode, all_tokenizers)
