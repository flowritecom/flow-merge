...
from flow_merge.lib.model_metadata import ModelMetadataService, FileMetadata, ModelMetadata
from flow_merge.lib.merge_settings import DirectorySettings


# High Priority Tests
def test_generate_content_hash():
    service = ModelMetadataService(token="", directory_settings=DirectorySettings(local_dir=Path(".")))
    ...
    assert actual_hash == expected_hash, "The generated content hash should match the expected hash"


@patch("huggingface_hub.hf_hub_download")
def test_download_hf_file(mock_download):
    service = ModelMetadataService(token="dummy_token", directory_settings=DirectorySettings(local_dir=Path(".")))
    ...
    mock_download.assert_called_once_with(
        "dummy_repo", "dummy_file", local_dir=Path("."), resume_download=True, token="dummy_token"
    )
...
def test_fetch_hf_model_info(mock_repo_info):
    ...
    mock_repo_info.assert_called_once_with(
        repo_id="dummy_repo", repo_type="model", files_metadata=True, token="dummy_token"
    )
...
def test_create_file_metadata_list_from_hf(mock_generate_content_hash, mock_download_hf_file):
    service = ModelMetadataService(token="dummy_token", directory_settings=DirectorySettings(local_dir=Path(".")))
    mock_generate_content_hash.return_value = "dummy_sha"
    mock_download_hf_file.return_value = "path/to/downloaded/file"

    sibling = RepoSibling(
        rfilename="dummy_file",
...
def test_load_model_info_hf(mock_create_file_metadata_list_from_hf, mock_fetch_hf_model_info):
    service = ModelMetadataService(token="dummy_token", directory_settings=DirectorySettings(local_dir=Path(".")))
    
    sibling = RepoSibling(rfilename="dummy_file", blob_id="dummy_blob_id", size=1234, lfs=None)
    model_info = ModelInfo(id="dummy_id", siblings=[sibling])
    ...
    mock_create_file_metadata_list_from_hf.assert_called_once_with(model_info, "dummy_repo")


@patch.object(ModelMetadataService, 'create_file_metadata_list_from_local')
@patch("transformers.AutoConfig.from_pretrained")
def test_load_model_info_local(mock_from_pretrained, mock_create_file_metadata_list_from_local):
    service = ModelMetadataService(token="", directory_settings=DirectorySettings(local_dir=Path("dummy_base_path/")))
    
    mock_create_file_metadata_list_from_local.return_value = []
    mock_from_pretrained.return_value.to_dict.return_value = {"config_key": "config_value"}
    ...
    mock_create_file_metadata_list_from_local.assert_called_once_with(Path("dummy_base_path/dummy_repo"))
...
@patch.object(ModelMetadataService, 'download_hf_file')
@patch.object(ModelMetadataService, 'generate_content_hash')
def test_end_to_end_hf_model(mock_generate_content_hash, mock_download_hf_file, mock_fetch_hf_model_info):
    service = ModelMetadataService(token="dummy_token", directory_settings=DirectorySettings(local_dir=Path(".")))
...
@patch.object(ModelMetadataService, 'create_file_metadata_list_from_local')
@patch("transformers.AutoConfig.from_pretrained")
def test_end_to_end_local_model(mock_from_pretrained, mock_create_file_metadata_list_from_local):
    service = ModelMetadataService(token="", directory_settings=DirectorySettings(local_dir=Path("dummy_base_path/")))
